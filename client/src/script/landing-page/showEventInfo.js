...
eventsContainer.addEventListener('click', (evento) =>{
    if(evento.target && evento.target.matches('.events__see-more')){//condicional para saber si si le está dando click al botón y si exsite un target
        ...
        eC2.forEach(element => {
            ...
            let eC = evento.target.parentNode.parentNode//obtenemos elemento padre
            let eC2 = eC.querySelectorAll('.events__see-more, .events__more-info, .events__see-less')//elemtnos
            eC2.forEach(element => {
              element.classList.remove('active')//qutiamos clase
          });
        }//luis-san
